...
function getCode(code) {
  ...
  case "ENGAGE_C_AXIS":
    ...
    return cAxisEngageModal.format(66);
  case "DISENGAGE_C_AXIS":
    machineState.cAxisIsEngaged = false;
    return cAxisEngageModal.format(67);
  ...
  case "START_CHIP_TRANSPORT":
    return mFormat.format(355);
  case "STOP_CHIP_TRANSPORT":
    return mFormat.format(356);
  case "OPEN_DOOR":
    return mFormat.format(61);
  case "CLOSE_DOOR":
    return mFormat.format(60);
...
/*
  ...
  case SPINDLE_PRIMARY: // main spindle
    if (machineState.isTurningOperation || machineState.axialCenterDrilling) { // turning main spindle
      ...
      writeBlock(
        //(machineState.tapping ? (mFormat.format(getCode("RIGID_TAPPING")), sOutput.format(tool.spindleRPM)) : sOutput.format(spindleSpeed)),
...
function onOpen() {
  ...
  if (properties.writeMachine && (vendor || model || description)) {
    //writeComment(localize("YOU JI YV-320E+C"));
    if (vendor) {
      writeComment(vendor);
    }
    if (model) {
      writeComment(localize("YV-320E+C W/ Fanuc Oi-TD"));
    }
    if (description) {
  ...
  writeBlock(mFormat.format(32));  //ensure "change 2nd spindle direction" is set to off
...
if (!machineState.axialCenterDrilling && !machineState.isTurningOperation) {
  lockCode = getCode("MAIN_SPINDLE_BRAKE_ON");
}
  ...
  if (true) {
    ...
    case "stop-boring":
      if (P > 0) {
        ...
        forceXYZ();
        writeBlock(
          gCycleModal.format((gPlaneModal.getCurrent() == 19) ? 89 : 85),
          getCommonCycle(x, y, z, cycle.retract),
          feedOutput.format(F)
        );
      }
