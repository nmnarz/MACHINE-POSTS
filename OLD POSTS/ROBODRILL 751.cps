...
function onOpen() {
  ...
  writeBlock(gFormat.format(53), "X" + xyzFormat.format(10.), "Y-"+xyzFormat.format(7.5));
...
function onSection() {
  ...
  if (insertToolCall ||
	...
	if (hasParameter("operation:cycleType") && (getParameter("operation:cycleType") != "drilling")){
    writeBlock(
      mFormat.format(tool.clockwise ? 3 : 4), sOutput.format(tool.spindleRPM) // added the G49 call to the beginning of the line
    );
	} else if (hasParameter("operation:cycleType") && (getParameter("operation:cycleType") != "tapping")){
    writeBlock(
      mFormat.format(tool.clockwise ? 3 : 4), sOutput.format(tool.spindleRPM) // added the G49 call to the beginning of the line
    );		
	} else {
	...
	}  //modified by MAN; this was to allow for spindle start right after tool change for the canned cycles.
