/**
  ...
  $Revision: 39185 $
  $Date: 2015-06-18 11:53:30 +0200 (to, 18 jun 2015) $
...
properties = {
  ...
  jobComment: "Fill Out Please",
  wcsLocation: "Top Center",
  xLocationDim: 10,
  yLocation: "",
  yLocationdim: 10,
  zLocation: "Stock Top",
...
function showDialog() {
  ...
  execute(findFile("Sauer_interactive_setup1.hta"), "\"" + path + "\"", false, "");
  var result = {};
  try {
...
/*     case "showblocknumbers":
      ...
      break; */
    case "wcsLocation":
      properties.wcsLocation = value;
      break;
	case "xOffsetDim":
	  properties.xLocationDim = value;
	  break;
	case "yOffsetDim":
	  properties.yLocationdim = value;
	  break;
	case "zLocation":
	  properties.zLocation = value;
	  break;
/*     case "tolerance":
...
function getProgramInfo() {
  ...
  for (var i = 0; i < numberOfSections; ++i) {
	...
	DepthofCut = section.getParameter("operation:stockZHigh") - section.getParameter("operation:surfaceZHigh");
...
function onOpen() {
  ...
  temporaryFolder = FileSystem.getTemporaryFile("!Sauer_Hagerman_SSExcel");
  // temporaryFolder = "D:\\setup-sheet-excel-2007"; // for testing
  
  FileSystem.removeFolderRecursive(temporaryFolder);

  var templatePath = FileSystem.getCombinedPath(getConfigurationFolder(), "!Sauer_Hagerman_SSExcel.xlsx");
  ...
  programInfo["program.jobComment"] = properties.jobComment;
  programInfo["program.xylocation"] = properties.wcsLocation;
  programInfo["program.zlocation"] = properties.zLocation;
  programInfo["program.xoffset"] = properties.xLocationDim;
  programInfo["program.yoffset"] = properties.yLocationdim;
...
function onSectionEnd() {
  ...
  //operationParameters["DepthofCut"] = DepthofCut / 25.4;
  if (properties.rapidFeed > 0) {
