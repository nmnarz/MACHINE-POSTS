...
function onSection() {
  ...
  if (!isFirstSection() && insertToolCall && properties.optionalStop && !manualNCStop) {
  ...
  if (isFirstCyclePoint()) {
    ...
    case "tapping":
      if (properties.useRigidTapping != "no") {
        writeBlock(mFormat.format(03), sOutput.format(spindleSpeed));
        writeBlock(mFormat.format(84), sOutput.format(spindleSpeed));
      }
    ...
    case "left-tapping":
      if (properties.useRigidTapping != "no") {
        writeBlock(mFormat.format(04), sOutput.format(spindleSpeed));
        writeBlock(mFormat.format(84), sOutput.format(spindleSpeed));
      }
      if (properties.usePitchForTapping) {
        writeBlock(
