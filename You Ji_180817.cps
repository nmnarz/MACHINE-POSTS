...
function getCode(code, spindle) {
  ...
  case "ENABLE_C_AXIS":
    ...
    return (spindle == SPINDLE_MAIN) ? 66 : "";
  case "DISABLE_C_AXIS":
    machineState.cAxisIsEngaged = true;
    return (spindle == SPINDLE_MAIN) ? 67 : "";
  ...
  case "STOP_SPINDLE":
    ...
    break;
  case "ORIENT_SPINDLE":
    return (spindle == SPINDLE_MAIN) ? 19 : "";
  ...
  case "LOCK_MULTI_AXIS":
    return (spindle == SPINDLE_MAIN) ? 54 : "";
  case "UNLOCK_MULTI_AXIS":
    return (spindle == SPINDLE_MAIN) ? 55 : "";
  case "CLAMP_CHUCK":
    return (spindle == SPINDLE_MAIN) ? 10 : "";
  case "UNCLAMP_CHUCK":
    return (spindle == SPINDLE_MAIN) ? 11 : "";
...
function onSection() {
  ...
  if (!isFirstSection() && insertToolCall &&
    ...
    if (stockTransferIsActive) {
      ...
      onCommand(COMMAND_COOLANT_OFF); //coolant off before homing NN
...
function onCyclePoint(x, y, z) {
  ...
  if (isFirstCyclePoint()) { // first cycle point
    ...
    case "chip-breaking":
      ...
      localZOutput.reset();
      writeBlock(
        gCycleModal.format(plane == 19 ? 87 : 83), // NN
        getCommonCycle(x, y, z, rapto, true),
