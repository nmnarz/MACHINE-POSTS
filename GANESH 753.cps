...
//merged with version 42072 8/17/18 NN
...
properties = {
  ...
  parametricTools: false, // tools call with macro
...
var coolants = {
  flood:  {on: [8, 48], off: [9]},
  mist: {},
  throughTool: {on: 48, off: 9},
  air: {},
  airThroughTool: {},
  suction: {},
  floodMist: {},
  floodThroughTool: {on: [8, 48], off: [9]},
  off: 9
};
...
function onOpen() {
  ...
  machineConfiguration.setDescription("M753");
  machineConfiguration.setVendor("GANESH");
  ...
  if (programName) {
    ...
    if (programComment) {
      writeln("O1010" + /*oFormat.format(programId) + */" (" + filterText(String(programId).toUpperCase(), permittedCommentChars) + ")");
  ...
  if (properties.writeTools) {
    ...
    if (tools.getNumberOfTools() > 0) {
      for (var i = 0; i < tools.getNumberOfTools(); ++i) {
        if (properties.parametricTools  == true) {
          var tool = tools.getTool(i);
          write("#" + toolFormat.format(tool.number+500));
          write("=" + tool.number);
          write(" ");
          var comment = getToolTypeName(tool.type) + " " + "D=" + xyzFormat.format(tool.diameter) + " " +
...
function onSection() {
  ...
  if (!isFirstSection() && properties.optionalStop && !manualNCStop) {
  ...
  if (insertToolCall) {
    ...
    disableLengthCompensation();//moved to below tool change position
    sequenceNumberForToolChange = true;
    ...
    if (properties.preloadTool) {
      ...
      if (nextTool) {
        if (properties.parametricTools) {
          writeBlock("T#" + toolFormat.format(nextTool.number + 500));
        } else {
      ...
      var firstToolNumber = section.getTool().number;
        if (tool.number != firstToolNumber) {
          if (properties.parametricTools) {
            writeBlock("T#" + toolFormat.format(firstToolNumber + 500));
          } else {
  ...
  // set coolant before we have positioned at Z
  setCoolant(tool.coolant);
  
  // start auger and chip evac coolant
  ...
  if (isFirstCyclePoint()) {
    ...
    /*case "circular-pocket-milling":
      ...
      writeBlock(gFormat.format(13), 
        ...
        "I" + xyzFormat.format(tool.diameter/2), "K" + xyzFormat.format(cycle.diameter/2),
        "Q" + xyzFormat.format(cycle.stepover), dOutput.format(d), feedOutput.format(F), "C" + xyzFormat.format(cycle.repeatPass));
      break;*/
    case "manual-boring":
      writeBlock(
  ...
  if (x || y || z) {
    if (pendingRadiusCompensation >= 0) {
      ...
      case RADIUS_COMPENSATION_LEFT:
        dOutput.reset();
        writeBlock(gMotionModal.format(1), gFormat.format(41), x, y, z, properties.parametricTools ? ("D#" + (d + 500))  : dOutput.format(d), f);
        break;
      case RADIUS_COMPENSATION_RIGHT:
        dOutput.reset();
        writeBlock(gMotionModal.format(1), gFormat.format(42), x, y, z, properties.parametricTools ? ("D#" + (d + 500))  : dOutput.format(d), f);
        break;
      default:
...
var manualNCStop = false; // manual NC type = stop
var mapCommand = {
  COMMAND_STOP:0,
...
function onClose() {
  ...
  writeRetract(Y); // return to home, Y only NN
