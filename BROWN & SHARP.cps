/**
  ...
  $Revision: 40108 $
  $Date: 2015-10-15 20:00:25 +0200 (to, 15 okt 2015) $
...
var milliFormat = createFormat({decimals:0, forceDecimal:true}); // milliseconds // range 1-9999
...
function onOpen() {
  ...
  if (properties.aAxisOn == true) { // note: setup your machine here
  ...
  if (programName) {
    ...
    //var oFormat = createFormat({width:(properties.o8 ? 8 : 4), zeropad:true, decimals:0});
		if (hasParameter("inventor:date") && hasParameter("inventor:user")){
			var dateStamp = getParameter("inventor:date");
			var user = getParameter("inventor:user");
			writeComment(programId + "		" + programComment);
		} else {
      writeComment(programId + " " + properties.Date + " " + properties.User);
  ...
  writeBlock(gPlaneModal.format(17), "E00 D00 H00");
  writeBlock(gFormat.format(40), gFormat.format(70), gAbsIncModal.format(90));
...
function onSection() {
  ...
  if (insertToolCall && !isFirstSection()){
	// writeBlock(mFormat.format(09));          // REMOVED THIS LINE TO REMOVE AN EXTRA M09 IN PROGRAM TIM CAFFERY 3-9-17
	 writeBlock(mFormat.format(05), mFormat.format(31));
  }
  ...
  if (insertToolCall) {
    ...
    writeBlock(gFormat.format(80), "H00", mFormat.format(6), "T" + toolFormat.format(tool.number), "(" + tool.comment + ")");
    if (tool.comment) {
	...
	writeBlock("E" + currentSection.workOffset, mFormat.format(3), mFormat.format(41), sOutput.format(tool.spindleRPM));
    if (properties.preloadTool) {
      var nextTool = getNextTool(tool.number);
      if (nextTool) {
        writeBlock(mFormat.format(32), hFormat.format(tool.lengthOffset), "T" + toolFormat.format(nextTool.number));
      } else {
        // preload first tool
		...
		if (insertToolCall) {
          writeBlock(mFormat.format(32), hFormat.format(tool.lengthOffset), "T" + toolFormat.format(firstToolNumber));
        } 
      }
    }
  }
  ...
  // set coolant after we have positioned at Z
   ...
   // setCoolant(tool.coolant);   //MOVED FURTHER DOWN TO TURN COOLANT ON LATER TIM CAFFERY 3-9-17
...
/*   if (!retracted) {
  ...
  if (insertToolCall || !lengthCompensationActive || retracted || (!isFirstSection() && getPreviousSection().isMultiAxis())) {
    ...
    if (machineConfiguration.isHeadConfiguration()) {
		var CompType = hasParameter("operation:compensationType") ? getParameter("operation:compensationType") : " ";
		var CompSize = hasParameter("operation:compensation") ? getParameter("operation:compensation") : " ";
		if (CompType == "control"){
    ...
    setCoolant(tool.coolant);   // MOVED BY TIM CAFFERY 3-8-17 TO TURN COOLANT ON LATER
  //writeBlock(gMotionModal.format(00), zOutput.format(initialPosition.z));
...
function onCyclePoint(x, y, z) {
  if (isFirstCyclePoint()) {
    ...
    var P = (cycle.dwell == 0) ? 0 : clamp(1, cycle.dwell, 99999999); // in milliseconds
    ...
    case "drilling":
      writeBlock(
        ...
        feedOutput.format(F), mFormat.format(08), gFormat.format(98)
      );
      break;
    case "counter-boring":
      if (P > 0) {
     ...
     //writeBlock(mFormat.format(29), sOutput.format(tool.spindleRPM));
      if (properties.usePitchForTapping) {
        writeBlock(
          ...
          getCommonCycle(x, y, z, cycle.retract),
          pitchOutput.format(tool.threadPitch), mFormat.format(08), gFormat.format(98)
    ...
    case "left-tapping":
      ...
      if (properties.usePitchForTapping) {
        ...
        writeBlock(
          gAbsIncModal.format(90), gCycleModal.format(74),
          getCommonCycle(x, y, z, cycle.retract),
          feedOutput.format(properties.useG95 ? tool.getTappingFeedrate()/spindleSpeed : tool.getTappingFeedrate()), mFormat.format(08), gFormat.format(98)
        );
      }
      break;
    case "right-tapping":
...
function onRapid(_x, _y, _z) {
  ...
  if (x || y || z) {
	...
	if (hasParameter("operation:operation_description") && getParameter("operation:operation_description") == "Drill"){
...
writeBlock(mFormat.format(5));
  ...
  //writeBlock(gFormat.format(00), "Z" + xyzFormat.format(0)); // retract
  ...
  if (!machineConfiguration.hasHomePositionX() && !machineConfiguration.hasHomePositionY()) {
    ...
    writeBlock(gFormat.format(00), "X" + xyzFormat.format(0), "Y" + xyzFormat.format(10)); // return to home
  } else {
    var homeX;
