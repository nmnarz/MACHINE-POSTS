...
var qFormat = createFormat({decimals:0, scale:1000}); //Scale x1000 NN
...
var peckOutput = createVariable({prefix:"Q", force:true}, qFormat); //Q Format for x1000 scale NN
...
var tailStockModal = createModal({}, mFormat);  // modal group M25-26 for tailstock NN
...
var machineState = { 
  tailstockIsActive: undefined  // machine state for tailstock NN
};
    ...
    case "chip-breaking":
      ...
      if (P > 0) {
        ...
        writeBlock(
          (activeGCodeList == 2) ? "" : gRetractModal.format(98), conditional(activeGCodeList != 2, gAbsIncModal.format(90)), gCycleModal.format(83),
          getCommonCycle(x, y, z, cycle.retract),
          peckOutput.format(cycle.incrementalDepth),  // format scale x1000 NN
          feedOutput.format(F)
        );
      }
      break;
...
function startSpindle(tappingMode, forceRPMMode, initialPosition, rpm) {
  ...
  if (tappingMode) {
    writeBlock(mFormat.format(329), sOutput.format(_spindleSpeed));
    return;
  } else {
